# CLI GUI-Modus Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Wenn die CLI-Binary per Doppelklick gestartet wird (Windows Explorer, macOS Finder, Linux Dateimanager), öffnet sich automatisch eine Browser-GUI statt des Terminals.

**Architecture:** `GuiDetector.java` erkennt per `ProcessHandle.parent()` ob der Start aus einem Dateimanager kommt. `GuiServer.java` startet einen eingebetteten `com.sun.net.httpserver.HttpServer` auf einem zufälligen Port, serviert eine vollständige Single-Page-App als Java-Textkonstante und stellt 7 JSON-API-Endpunkte bereit. `CLI.main()` delegiert an `GuiServer.start()` wenn GUI-Modus erkannt wird.

**Tech Stack:** Java 21 Text Blocks, `com.sun.net.httpserver` (JDK built-in, keine neue Dependency), Jackson 2.18 (bereits vorhanden), GraalVM native-image 21 CE
...
| Datei | Änderung |
|-------|---------|
| `src/de/kreisalarm/cli/GuiDetector.java` | **Neu** |
| `src/de/kreisalarm/cli/GuiServer.java` | **Neu** |
| `src/de/kreisalarm/cli/CLI.java` | `main()` Zeile 31–38 anpassen |
| `build.bat` | Zeile 122: zwei neue Dateien in `javac` |
| `build-linux.sh` | Zeilen 18–24: zwei neue Dateien in `javac` |
| `build-macos.sh` | Zeilen 35–41: zwei neue Dateien in `javac` |

---

### Task 1: GuiDetector.java

**Files:**
- Create: `src/de/kreisalarm/cli/GuiDetector.java`
...
- [ ] **Step 2: Kompilieren (nur diese Datei isoliert testen)**

Windows:
```bat
cd D:\javasrc\workspace\kreisalarm.de\CLI
mkdir tmp\classes 2>nul
graalvm-win\bin\javac --release 21 -cp lib\jackson\* -d tmp\classes ^
  src\de\kreisalarm\cli\Config.java ^
  src\de\kreisalarm\cli\RestClient.java ^
  src\de\kreisalarm\cli\TablePrinter.java ^
  src\de\kreisalarm\cli\CLI.java ^
  src\de\kreisalarm\cli\GuiDetector.java
```

Erwartet: kein Fehler, `tmp\classes\de\kreisalarm\cli\GuiDetector.class` existiert.

- [ ] **Step 3: Commit**

```bash
git add src/de/kreisalarm/cli/GuiDetector.java
git commit -m "feat: GuiDetector – Doppelklick vs Terminal erkennen"
```

---

### Task 2: GuiServer.java

**Files:**
- Create: `src/de/kreisalarm/cli/GuiServer.java`

Enthält: HTTP-Server-Scaffold, vollständiges HTML/CSS/JS als Java-Textkonstante (Single-Page-App), 7 API-Handler.

- [ ] **Step 1: GuiServer.java anlegen**

Die Datei ist groß (~500 Zeilen mit eingebettetem HTML). Vollständiger Inhalt:
...
graalvm-win\bin\javac --release 21 -cp lib\jackson\* -d tmp\classes ^
  ...
  src\de\kreisalarm\cli\TablePrinter.java ^
  src\de\kreisalarm\cli\GuiDetector.java ^
  src\de\kreisalarm\cli\GuiServer.java ^
  src\de\kreisalarm\cli\CLI.java
```

Erwartet: kein Fehler.

- [ ] **Step 3: JAR bauen und GUI testen**

```bat
mkdir tmp\fat 2>nul
cd tmp\fat
for %f in (..\..\lib\jackson\jackson-core-*.jar) do graalvm-win\bin\jar xf %f
for %f in (..\..\lib\jackson\jackson-annotations-*.jar) do graalvm-win\bin\jar xf %f
for %f in (..\..\lib\jackson\jackson-databind-*.jar) do graalvm-win\bin\jar xf %f
cd ..\..\
xcopy /s /e /q tmp\classes\* tmp\fat\ >nul
echo Main-Class: de.kreisalarm.cli.CLI> tmp\MANIFEST.MF
echo.>> tmp\MANIFEST.MF
graalvm-win\bin\jar cfm tmp\meindrk-cli.jar tmp\MANIFEST.MF -C tmp\fat .
```

Dann GUI starten (erzwingt GUI-Modus mit `--gui`):
```bat
graalvm-win\bin\java -jar tmp\meindrk-cli.jar --gui
```

Erwartet:
- Browser öffnet sich auf `http://localhost:<port>`
- Login-Formular erscheint
- KV-Dropdown befüllt sich mit Einträgen von `https://meindrk.team`
- Login mit gültigen Zugangsdaten funktioniert
- Befehle (z. B. `person list`) liefern Ergebnisse in der Tabelle

- [ ] **Step 4: Commit**

```bash
git add src/de/kreisalarm/cli/GuiServer.java
git commit -m "feat: GuiServer – eingebetteter HTTP-Server mit Browser-GUI"
```

---

### Task 3: CLI.java — GUI-Erkennung einbauen

**Files:**
- Modify: `src/de/kreisalarm/cli/CLI.java` (Zeile 31)

- [ ] **Step 1: `main()` anpassen**

Aktuelle Zeilen 31–38 in `CLI.java`:
```java
...
- [ ] **Step 3: Testen — Terminal-Modus bleibt unberührt**

```bat
graalvm-win\bin\java -jar tmp\meindrk-cli.jar person list --help
```

Erwartet: normaler CLI-Output (kein Browser öffnet sich).

- [ ] **Step 4: Testen — `--no-gui` erzwingt Terminal**

```bat
graalvm-win\bin\java -jar tmp\meindrk-cli.jar --no-gui
```

Erwartet: `printHelp()` gibt Hilfe aus, kein Browser.

- [ ] **Step 5: Commit**

```bash
git add src/de/kreisalarm/cli/CLI.java
git commit -m "feat: CLI.main – GUI-Modus bei Doppelklick aktivieren"
```

---

### Task 4: Build-Skripte aktualisieren

**Files:**
- Modify: `build.bat` (Zeile 122)
- Modify: `build-linux.sh` (Zeilen 18–24)
- Modify: `build-macos.sh` (Zeilen 35–41)

- [ ] **Step 1: `build.bat` — Zeile 122 anpassen**
...
- [ ] **Step 2: `build-linux.sh` — Zeilen 18–24 anpassen**
...
- [ ] **Step 3: `build-macos.sh` — Zeilen 35–41 anpassen**
...
- [ ] **Step 4: Vollständigen Windows-Build testen**

```bat
build.bat
```

Erwartet: `build\meindrk-cli-windows-x64.exe` wird erstellt. Doppelklick darauf öffnet Browser mit GUI.

- [ ] **Step 5: Commit**

```bash
git add build.bat build-linux.sh build-macos.sh
git commit -m "build: GuiDetector und GuiServer in javac-Aufruf ergaenzen"
```
